	...
	internal class RequestObserver : IObserver<UrlRequest>
	{
		...
		public RequestObserver(Client client, RequestOptions options)
		{
			...
			_downloadRequired
				...
				.Select(reqs =>
				    new EmbedlyRequest(
				        new Uri(@"https://api.embed.ly/1/oembed?format=json&key=" + client.Key + @"&urls=" + string.Join(",", reqs.Select(req => Uri.EscapeDataString(req.Url.OriginalString))) + options.GetQueryString()),
				        reqs
				    )
				)
